...
- **Flocking / social movement**: two config-gated behaviors layer onto the wander via a velocity-steering engine. `flocking` (default **on**) makes mascots loosely group — separation (no overlap) + cohesion (gather) + alignment — and, when two idle mascots come within `meetDistance`, both stop and face each other for 1–2 s (a wordless greeting, then a `meetCooldownRange` gap). `cursor_gather` (default **off**, with a `radius`) makes mascots within the radius steer to the mouse (`NSEvent.mouseLocation`), leaving the roaming area to reach it and drifting back when it leaves. The force math is pure and unit-tested in `Flocking.swift` (`separation`/`cohesion`/`alignment`/`cursorSeek`/`nearestMeetTarget`/`steer`, all AppKit-free, operating on `NeighborState` in global screen coords). `CharacterController.tick()` picks by mode priority **meeting → cursor-seek → wander(+flock)** via `steeringTick()`, and falls back to the unchanged legacy target-wander (`legacyWanderTick`) when both toggles are off — so "both off" is byte-for-byte today's behavior. `MascotManager.neighborStates(excluding:)` injects other mascots' snapshots (center/velocity/engageable) into each controller through its `neighbors` closure; `working`/frozen/dragged/hidden mascots are excluded from both flocking and greetings. Config keys `flocking` / `cursor_gather` hot-reload via `setFlocking` / `setCursorGather` (mirrors `setMovement`/`setSpeed`); tuning constants (perception/weights/meet/cursor) are `private static` on `CharacterController`. A mascot's greeting stays mutual because a meeting mascot remains `engageableForMeeting`, so the other one also turns to face it
