...
**Sessions tracking**: `SessionTracker` (in `EventManager.sessionTracker`) is a small ObservableObject that records per-session `SessionInfo { id, pid, cwd, name, state, lastSeen }` on every incoming event (`name` is the last auto name from the hook). Sorted by `lastSeen` desc and exposed as `@Published var sessions` for the menu bar popover. `pruneDeadProcesses` runs every 10s — sessions whose pid no longer exists (`kill(pid, 0) != 0`) are removed. If the global state was `working` and no session remains in `working` after a prune, EventManager forces an `idle` transition (so the mascot doesn't get stuck working when Claude Code crashes).
